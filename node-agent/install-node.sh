...
# ---------- connect to the panel automatically (master install) ----------
# Logs into the panel's API and registers THIS node via its connect key, so the
# user does NOT have to click anything in the panel UI.
# Inputs (args/env): PANEL_URL, PANEL_USER, PANEL_PASS  (or answered prompts)
esc_json() {
  # Minimal, dependency-free JSON string escaper (bash)
  local s="$1"
  s="${s//\\/\\\\}"
  s="${s//\"/\\\"}"
  printf '%s' "$s"
...
connect_to_panel() {
  local KEY_STR="${JOIN_CODE}@${NODE_HOST}:${AGENT_PORT}"
  if [[ -n "$TUN_URL" ]]; then
    # If we built a tunnel, tell the panel to reach the node through it.
    KEY_STR="${JOIN_CODE}@${TUN_URL#https://}:443"
  fi

  echo ""
  echo "${BOLD}=== Connect this node to your Venlix panel automatically ===${NC}"
  if [[ -z "$PANEL_URL" ]]; then
    read -r -p "  Panel URL (e.g. https://panel.example.com): " PANEL_URL
  fi
  PANEL_URL="${PANEL_URL%/}"
  if [[ -z "$PANEL_URL" ]]; then
    echo "    ${YELLOW}[!] No panel URL — skipping auto-connect. Use the key printed above in the panel UI.${NC}"
    return 0
  fi
  if [[ -z "$PANEL_USER" ]]; then
    read -r -p "  Panel admin username: " PANEL_USER
  fi
  if [[ -z "$PANEL_PASS" ]]; then
    read -r -s -p "  Panel admin password: " PANEL_PASS; echo ""
  fi
  if [[ -z "$PANEL_USER" ]] || [[ -z "$PANEL_PASS" ]]; then
    echo "    ${YELLOW}[!] Missing panel login — skipping auto-connect.${NC}"
    return 0
  fi

  local LOGIN="${PANEL_URL}/api/auth/login"
  local REG="${PANEL_URL}/api/admin/nodes/connect-key"
  echo "    Logging into panel API ($PANEL_URL)..."
  local BODY_LOGIN="{\"username\":\"$(esc_json "$PANEL_USER")\",\"password\":\"$(esc_json "$PANEL_PASS")\"}"
  local BODY_KEY="{\"key\":\"$(esc_json "$KEY_STR")\"}"
  local RESP TOKEN
  RESP="$(curl -s -m 15 -X POST "$LOGIN" -H 'Content-Type: application/json' -d "$BODY_LOGIN")"
  TOKEN="$(printf '%s' "$RESP" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')"
  if [[ -z "$TOKEN" ]]; then
    echo "    ${RED}[x] Login failed. Check panel URL/user/password.${NC}"
    echo "    Fallback: paste this key in the panel UI -> Connect With Node Key:"
    echo "      $KEY_STR"
    return 1
  fi

  echo "    Registering node with the panel..."
  local OUT ER
  OUT="$(curl -s -m 20 -X POST "$REG" -H 'Content-Type: application/json' -H "Authorization: Bearer $TOKEN" -d "$BODY_KEY")"
  if printf '%s' "$OUT" | grep -qi 'error'; then
    ER="$(printf '%s' "$OUT" | sed -n 's/.*"error":"\([^"]*\)".*/\1/p')"
    echo "    ${YELLOW}[!] Panel responded: ${ER:-unknown error}.${NC}"
    echo "    Fallback: paste this key in the panel UI -> Connect With Node Key:"
    echo "      $KEY_STR"
  else
    echo "    ${GREEN}[+] Node registered in the panel successfully!${NC}"
    echo "    It is now online under: $NODE_NAME (reaching via $KEY_STR)"
  fi
}

# Only run auto-connect if env says so, or if this is an interactive run.
if [[ "$PANEL_AUTO_CONNECT" != "0" ]]; then
  connect_to_panel || true
fi
echo ""
